...
export class TypeScriptParser {
  async parse(content: string, fileId: string): Promise<ParseResult> {
    ...
    try {
      ...
      traverse(ast, {
          ...
          if (path.node.id) {
            ...
            const classNode: KnowledgeNode = {
              ...
              properties: {
                ...
                isAbstract: this.isAbstract(path.node),
                extends: this.getExtends(path.node),
                implements: this.getImplements(path.node)
              }
            };
        ...
        TSInterfaceDeclaration(path) {
          if (path.node.id) {
            ...
            const interfaceNode: KnowledgeNode = {
              id: this.generateId(`${fileId}:${interfaceName}`),
              type: 'class', // Neo4j doesn't have interface type, use class
              name: interfaceName,
              properties: {
                file: fileId,
                line: path.node.loc?.start.line,
                kind: 'interface',
                isExported: this.isExported(path.parentPath),
                extends: this.getInterfaceExtends(path.node)
              }
            };
          ...
          if (path.node.id) {
            ...
            const functionNode: KnowledgeNode = {
              ...
              properties: {
                ...
                returnType: this.getReturnType(path.node)
              }
            };
      ...
      throw new Error(`TypeScript parsing failed: ${(error as Error).message}`);
    }
  ...
  private isAbstract(node: any): boolean {
    return node.modifiers?.some((mod: any) => mod.type === 'TSAbstractKeyword') || false;
  }
  ...
  private getImplements(node: any): string[] {
    return node.implements?.map((impl: any) => (impl.expression as any).name) || [];
  }

  private getInterfaceExtends(node: any): string[] {
    return node.extends?.map((ext: any) => (ext.expression as any).name) || [];
  }

  private getReturnType(node: any): string | null {
    return node.returnType ? (node.returnType.typeAnnotation as any).type : null;
  }
